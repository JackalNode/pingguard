...
> Last updated: Session 18 — Apex Legends and Path of Exile audited using the trace_connections.py + WHOIS methodology. Apex: CDN-shadowing bug fixed, endpoint replaced with WHOIS-confirmed AWS address; live testing revealed dynamic per-match datacenter assignment as a confirmed, generalizable limitation. Path of Exile: same CDN-shadowing pattern found and fixed with a WHOIS-confirmed Google Cloud (South Africa-routed) endpoint, matched closely against both the in-game gateway ping (22ms) and live in-app testing (27ms vs 32ms tcp_ping) — the tightest confirmation of any fix this session. PoE's stale exe value (PathOfExile.exe vs the real PathOfExileSteam.exe) also fixed, which required building migrate_game_endpoints() out to support exe-field migrations for the first time — a capability that didn't exist in the codebase before tonight. Separately, confirmed PingGuard's "missing games" symptom was not a bug: only 6 of 21 DEFAULT_GAMES entries were enabled in the dev's own games.json. All 21 set to enabled for ongoing testing — dev intends to leave them that way.
...
- 🔲 Audit remaining default games for the same blended-region issue — Warzone, Apex, and Path of Exile completed (Sessions 17–18) using the proven `trace_connections.py` + WHOIS method. World of Warcraft flagged as the next candidate, already enabled in-app and ready to test immediately. ~13 default games remain unaudited beyond that.
...
- World of Warcraft (`eu.actual.battle.net:1119`) remains unaudited — Warzone, Apex, and Path of Exile completed across Sessions 17–18 using the same proven method. WoW is already enabled in the dev's `games.json`, ready to test immediately next session.
